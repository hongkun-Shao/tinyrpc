...
TcpConnection::TcpConnection(
    ...
    : m_event_loop_(event_loop),
      m_peer_addr_(peer_addr),
      m_local_addr_(local_addr),
      m_state_(NotConnected),
      m_fd_(fd),
...
void TcpConnection::OnRead() {
  ...
  if (m_state_ != Connected) {
    ERRORLOG(
        "onRead error, client has already disconneced, addr[%s], clientfd[%d]",
        m_peer_addr_->ToString().c_str(), m_fd_);
    return;
  }
  ...
  if (m_connection_type_ == TcpConnectionByServer) {
    ...
    for (size_t i = 0; i < result.size(); ++i) {
      ...
      INFOLOG("success get request[%s] from client[%s]",
              result[i]->m_msg_id_.c_str(), m_peer_addr_->ToString().c_str());
    ...
    // decode the char stream in buffer, get the message object, and excute the
    // callback done
...
void TcpConnection::set_state(const TcpState state) { m_state_ = Connected; }

TcpState TcpConnection::get_state() { return m_state_; }
...
void TcpConnection::ListenRead() {
  m_fd_event_->Listen(FdEvent::IN_EVENT,
