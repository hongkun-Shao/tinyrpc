...
TcpClient::TcpClient(NetAddr::s_ptr peer_addr) : m_peer_addr_(peer_addr) {
  ...
  m_connection_ = std::make_shared<TcpConnection>(
      m_event_loop_, m_fd_, 128, peer_addr, nullptr, TcpConnectionByClient);
...
void TcpClient::Connect(std::function<void()> done) {
  int res =
      connect(m_fd_, m_peer_addr_->GetSockAddr(), m_peer_addr_->GetSockLen());
  if (res == 0) {
    ...
    if (errno == EINPROGRESS) {
      ...
      m_fd_event_->Listen(FdEvent::OUT_EVENT, [this, done]() {
        int rt = ::connect(m_fd_, m_peer_addr_->GetSockAddr(),
                           m_peer_addr_->GetSockLen());
        if ((rt < 0 && errno == EISCONN) || (rt == 0)) {
          ...
          if (errno == ECONNREFUSED) {
            ...
            m_connect_error_info_ =
                "connect refused, sys error = " + std::string(strerror(errno));
          } else {
            m_connect_error_code_ = ERROR_FAILED_CONNECT;
            m_connect_error_info_ = "connect unkonwn error, sys error = " +
...
void TcpClient::ReadMessage(const std::string& msg_id,
                            std::function<void(AbstractProtocol::s_ptr)> done) {
  // start listen read
  // decode buffer data to get message object, check msg_id equal msg_id, if
  // equal, do callback
...
int TcpClient::getConnectErrorCode() { return m_connect_error_code_; }

std::string TcpClient::getConnectErrorInfo() { return m_connect_error_info_; }

NetAddr::s_ptr TcpClient::getPeerAddr() { return m_peer_addr_; }

NetAddr::s_ptr TcpClient::getLocalAddr() { return m_local_addr_; }
