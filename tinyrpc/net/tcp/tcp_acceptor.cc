...
TcpAcceptor::TcpAcceptor(NetAddr::s_ptr local_addr)
  ...
  int val = 1;
  if (setsockopt(m_listenfd_, SOL_SOCKET, SO_REUSEADDR, &val, sizeof(val)) !=
      0) {
...
int TcpAcceptor::get_listenfd() { return m_listenfd_; }
...
std::pair<int, NetAddr::s_ptr> TcpAcceptor::Accept() {
  if (m_family_ == AF_INET) {
    ...
    int client_fd =
        accept(m_listenfd_, reinterpret_cast<sockaddr*>(&client_addr),
               &client_addr_len);
    if (client_fd < 0) {
