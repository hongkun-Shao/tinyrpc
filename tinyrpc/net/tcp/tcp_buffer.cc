...
void TcpBuffer::MoveReadIndex(int offset){
    ...
    if (j >= m_buffer_.size()) {
        ERRORLOG("moveReadIndex error, invalid offset %d, old_read_index %d, buffer size %d", offset, m_read_index_, m_buffer_.size());
        return;
    }
    m_read_index_ = j;
    AdjustBuffer();
...
void TcpBuffer::MoveWriteIndex(int offset){
    ...
    if (j >= m_buffer_.size()) {
        ERRORLOG("moveWriteIndex error, invalid offset %d, old_read_index %d, buffer size %d", offset, m_read_index_, m_buffer_.size());
        return;
    }
    m_write_index_ = j;
    AdjustBuffer();
