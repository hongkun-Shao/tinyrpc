...
void EventLoop::Loop() {
  ...
  while (!m_stop_flag_) {
    ...
    int res =
        epoll_wait(m_epoll_fd_, result_events, g_epoll_max_events_num, timeout);
    DEBUGLOG("now end epoll_wait, res = %d", res);

    if (res < 0) {
      ERRORLOG("epoll_wait error, errno = %d, error = %s", errno,
               strerror(errno));
    } else {
      for (int i = 0; i < res; ++i) {
...
bool EventLoop::IsInLoopThread() { return GetThreadId() == m_thread_id_; }
...
bool EventLoop::IsLooping() { return m_is_looping_; }
