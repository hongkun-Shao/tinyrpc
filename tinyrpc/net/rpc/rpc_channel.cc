...
void RpcChannel::CallMethod(const google::protobuf::MethodDescriptor* method,
  ...
  INFOLOG("%s | call method name [%s]", req_protocol->m_msg_id_.c_str(),
          req_protocol->m_method_name_.c_str());

  if (!m_is_init_) {
    std::string err_info = "RpcChannel not init";
    my_controller->SetError(ERROR_RPC_CHANNEL_INIT, err_info);
    ERRORLOG("%s | %s, RpcChannel not init ", req_protocol->m_msg_id_.c_str(),
             err_info.c_str());
    return;
  }
  ...
  if (!request->SerializeToString(&(req_protocol->m_pb_data_))) {
    ...
    ERRORLOG("%s | %s, origin requeset [%s] ", req_protocol->m_msg_id_.c_str(),
  ...
  m_timer_event_ = std::make_shared<TimerEvent>(
    ...
    channel->getTcpClient()->WriteMessage(
        req_protocol, [req_protocol, channel,
          ...
          channel->getTcpClient()->ReadMessage(
              req_protocol->m_msg_id_,
              [channel, my_controller](AbstractProtocol::s_ptr msg) mutable {
                std::shared_ptr<tinyrpc::TinyPBProtocol> rsp_protocol =
                    std::dynamic_pointer_cast<tinyrpc::TinyPBProtocol>(msg);
                INFOLOG(
                    "%s | success get rpc response, call method name[%s], peer "
                    "addr[%s], local addr[%s]",
                    rsp_protocol->m_msg_id_.c_str(),
                    rsp_protocol->m_method_name_.c_str(),
                    channel->getTcpClient()->getPeerAddr()->ToString().c_str(),
                ...
                if (rsp_protocol->m_err_code_ != 0) {
                  ERRORLOG(
                      ...
                      rsp_protocol->m_msg_id_.c_str(),
                      rsp_protocol->m_method_name_.c_str(),
                      rsp_protocol->m_err_code_,
                      rsp_protocol->m_err_info_.c_str());

                  my_controller->SetError(rsp_protocol->m_err_code_,
...
google::protobuf::Message* RpcChannel::getRequest() { return m_request_.get(); }

google::protobuf::Message* RpcChannel::getResponse() {
  return m_response_.get();
}

google::protobuf::Closure* RpcChannel::getClosure() { return m_closure_.get(); }

TcpClient* RpcChannel::getTcpClient() { return m_client_.get(); }

TimerEvent::s_ptr RpcChannel::getTimerEvent() { return m_timer_event_; }
