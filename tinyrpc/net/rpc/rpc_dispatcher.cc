...
void RpcDispatcher::dispatch(AbstractProtocol::s_ptr request, AbstractProtocol::s_ptr response, TcpConnection* connection){
    ...
    if (method == NULL) {
        ERRORLOG("%s | method neame[%s] not found in service[%s]", req_protocol->m_msg_id_.c_str(), method_name.c_str(), service_name.c_str());
    ...
    if (!req_msg->ParseFromString(req_protocol->m_pb_data_)) {
        ERRORLOG("%s | deserilize error", req_protocol->m_msg_id_.c_str(), method_name.c_str(), service_name.c_str());
    ...
    INFOLOG("%s | get rpc request[%s]", req_protocol->m_msg_id_.c_str(), req_msg->ShortDebugString().c_str());
    ...
    if(!rsp_msg->SerializeToString(&(rsp_protocol->m_pb_data_))){
        ERRORLOG("%s | serilize error, origin message [%s]", req_protocol->m_msg_id_.c_str(), rsp_msg->ShortDebugString().c_str());
    ...
    INFOLOG("%s | dispatch success, requesut[%s], response[%s]", req_protocol->m_msg_id_.c_str(), req_msg->ShortDebugString().c_str(), rsp_msg->ShortDebugString().c_str());
